...
- Lists of options, categories, dropdowns, select fields → "pickList"
- File uploads, attachments → "file"
- JSON data, complex structures → "json"
- References to other objects → "relation"

**PickList Detection:**
- Look for fields with predefined options or choices
- Identify lists of values like "Male, Female", "Active, Inactive", etc.
- Consider dropdown, select, option, choice keywords as pickList indicators
- Extract specific values mentioned by the user for pickList items
