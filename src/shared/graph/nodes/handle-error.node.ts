...
@Injectable()
export class GenericHandleErrorNode<TState extends GraphStateBase> extends BaseGraphNode<TState> {
  constructor(
  ...
  execute(state: TState): Partial<TState> {
    const result = this.handlerService.handleError(state, this.config, this.additionalData);
