...
@Injectable()
export class GenericHandleRetryNode<TState extends GraphStateBase> extends BaseGraphNode<TState> {
  constructor(
  ...
  execute(state: TState): Partial<TState> {
    const result = this.handlerService.handleRetry(state, this.config, this.additionalData);
